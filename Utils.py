...
def lr_images(images_real , downscale):
    ...
    for img in  range(len(images_real)):
        images.append(array(Image.fromarray(images_real[img]).resize([images_real[img].shape[0]//downscale,images_real[img].shape[1]//downscale],resample=PIL.Image.BICUBIC)))
...
def evaluate(x_test_hr, x_test_lr,generator):
    ...
    mse_n = 0.0
    psnr  = 0.0
    for index in range(x_test_hr.shape[0]):
